...
def check_argument(datasetname,genfragment,nevents,nsplitjobs,inputname):
        if not datasetname.endswith("_FS"):
          print "[EXIT] Recommend to put '_FS' at the end of the DATASETNAME"
          print "[EXIT] e.g.) DYJetsToLL -> DYJetsToLL_FS"
...
if "16" in campaign:
  pileup_input = "dbs:/Neutrino_E-10_gun/RunIIFall17FSPrePremix-PUFSUL16CP5_106X_mcRun2_asymptotic_v16-v1/PREMIX"
elif "17" in campaign:
...
for list_l in list_ls:
  ...
  cmsdriver_l = "cmsDriver.py Configuration/GenProduction/python/"+datasetname+".py --no_exec --mc --python_filename run_crab.py --fileout "+step+".root --eventcontent AODSIM --fast --datatier AODSIM --step GEN,SIM,RECOBEFMIX,DIGI,DATAMIX,L1,DIGI2RAW,L1Reco,RECO --procModifiers premix_stage2 --datamix PreMix -n 6284 --pileup_input "+pileup_input+" "+add_cmsdriver
  if inputLHE:
