...
if ($#ARGV!=1) {
    ...
    print "    --nEventsBatch (number of events per job) (default $nEvents)\n";
    print "    --totalEvents (total numbers of events to run on) (default $totalEvents)\n";
...
while ($i<$totalEvents){
    ...
    $jobCfg=specializeCfgNevents($cfg,$ii,$i);
...
sub specializeCfgNevents($$@) {
    my ($inp, $index, $skipEvents)=@_;
    ...
    while(<INP>) {
        ...
        if ($sector==2 && /^[^\#]*fileName\s*=/) {
            ...
            # PoolOutputModule Block
        } elsif ($sector==1 && /^[^\#]*skipEvents\s*=/) {
	    print OUTP "    skipEvents = cms.untracked.uint32($skipEvents),\n";
